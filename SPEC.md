...
Interview coaching may include an optional `sampleAnswer` as a separate illustrative example. `candidateAnswer` remains the factual source; `improvedAnswer` is a grounded rewrite using only that source, while `sampleAnswer` may use explicitly hypothetical details and is never candidate evidence or input to scores, strengths, transcript, reports, skills, progress or recommendations. Its framework is `star`, `self_intro`, `technical` or `direct`; STAR is reserved for behavioral/past-experience questions, not forced on introductions or technical explanations. An unusable sample is omitted/null without invalidating otherwise-valid evaluation, another AI call, or a change to the configured token budget. See [SRS FR-INT-08](docs/SRS.md#64-mock-interview-and-report) and the [API contract](docs/03-api-data-contract.md#trả-lời-interview).
