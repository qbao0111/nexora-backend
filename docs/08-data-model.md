...
Activity statuses are pending, completed and obsolete; only pending-to-completed is exposed by the API. Refresh reconciliation is additive: completed activities are never deleted or reset, pending activities no longer required become obsolete, and new gaps become pending rows. The unique activity key and user-serialized transaction make initial generation and refresh idempotent under repeated/concurrent requests. Published Scenario IDs are selected server-side; B11 never accepts arbitrary resource IDs or URLs from the client.
