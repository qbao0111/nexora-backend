...
`user_profiles.primary_resume_id` is a nullable owner-side reference to `resumes.id`, with an index and a `SET NULL` foreign key action when a resume is deleted. The service only permits an authenticated owner to select a `ready` resume, so each user has zero or one explicit Primary Resume without an `IsPrimary` column on every resume. The MVP does not auto-select a newly uploaded resume; the client calls `PUT /api/v1/me/primary-resume` after extraction is ready. Career Profile is an aggregate read model, not a new table: `GET /api/v1/me/career-profile` composes identity/profile, the explicit Primary Resume, that resume's latest analysis, active Career Goal, computed Skill Profile summary, and the Learning Path for the active goal. It is read-only, does not create a path, and does not copy CV, AI, or practice content. Onboarding `isComplete` requires only Primary Resume and active Career Goal.
