...
- Production fail-closed: internal-development Gemini/DeepSeek AI, Fake Payment and the development upload adapter cannot be enabled in Production. `R2StorageProvider` plus the durable A2 upload-intent/finalize path is accepted only with validated private R2 configuration; `Storage:Provider=local` is rejected regardless of the upload feature flag. DEC-01, DEC-02, DEC-03 and DEC-04 still gate the corresponding production enablement.
