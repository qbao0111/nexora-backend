...
| AI | `IAiProvider` adapter; `FakeAiProvider` và configuration-driven development `GeminiAiProvider` được phép trước DEC-01; timeout, bounded retry, structured schema/semantic validation, token/cost telemetry; no client key/provider type leak. |
...
Các quyết định dưới đây **không block backend/local development, Phases 0–3 hoặc integration test dùng fake/development adapters**. Chúng chỉ block real production capability tương ứng:
...
Pricing/plan business values vẫn phải được xác định theo ticket liên quan nhưng không phải lý do để block repository/foundation work. Không suy ra production decision từ Gemini, fake provider, local storage hoặc vendor example.
