...
| ID | Requirement | Priority | Verification |
| --- | --- | --- | --- |
| FR-INT-01 | Candidate chọn role, seniority, interview type, difficulty, ngôn ngữ tùy chọn (`vi-VN`/`en-US`) và tuỳ chọn CV/JD trước khi start; nếu bỏ qua ngôn ngữ, server dùng `vi-VN`. | Must | Invalid/foreign CV-JD ID hoặc language ngoài allowlist bị từ chối; session lưu ngôn ngữ server-owned. |
