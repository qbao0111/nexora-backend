...
- **Validation integrity and operation ownership**:
  ...
  - `interview.report` grounding repair treats submitted answers as the only evidence. Questions, CV/profile/JD context and low-information answers are not positive candidate evidence. After terminal `AI_OUTPUT_INVALID` only, report generation may aggregate persisted validated answer evaluations deterministically, then must pass the same report validator. No safe grounded strength means fail closed; transport/auth/config/rate-limit failures never select this fallback.
