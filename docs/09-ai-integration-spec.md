...
- **Repair Cycle**: If attempt 1 fails server semantic validation with a repairable issue, attempt 2 injects a focused repair prompt specifying the exact contract violation and instructions to correct it. If provider content is malformed JSON or valid JSON that cannot deserialize to the typed contract, the provider emits `MalformedStructuredOutput`; the same single retry uses strict JSON-correction instructions while the original untrusted input remains a separate user message.
...
- **Error Normalization**: Maps failures to canonical `BusinessException` with `BusinessErrorKind.ExternalFailure` and safe error codes (`AI_OUTPUT_INVALID`, `AI_RATE_LIMITED`, `AI_PROVIDER_UNAVAILABLE`, `AI_PROVIDER_AUTH_FAILED`, `AI_PROVIDER_CONFIGURATION_INVALID`). Authentication and configuration failures are terminal and retain their provenance; they never normalize to `AI_OUTPUT_INVALID` or select deterministic output fallback.
...
`RepairUsed` means that attempt 2 used either Nexora semantic repair instructions or strict malformed-JSON correction instructions. Reasoning-effort and truncation retries remain provider-policy fallbacks and report `RepairUsed=false`. Every retry stays inside the same two-call budget.
...
For `interview.report` only, a DeepSeek semantic-repair second attempt disables thinking while retaining the 6,000-token output cap, original input/schema, and global two-call ceiling. This prevents reasoning tokens from exhausting the repair response budget; the configured low-thinking policy still applies to attempt one. No other operation or provider selection changes.

The trusted system message contains operation metadata, the approved instructions and the exact Nexora-owned JSON schema. The untrusted CV/JD/answer/scenario text is sent only as the user message. The adapter requires nonblank `choices[0].message.content`, parses it first as JSON, then deserializes the parsed root to the operation contract. Parse failures use stage `content_json_parse_failed`; typed contract mismatches use `content_contract_deserialization_failed`. Both carry `MalformedStructuredOutput` unless authoritative truncation/reasoning-exhaustion metadata selects its existing hint. The adapter ignores `reasoning_content`, never strips fences or fabricates defaults, and does not copy provider content into exceptions.
...
- **Validation integrity and operation ownership**:
  ...
  - Missing required semantic content is repairable once through the structured executor; a second invalid response becomes the normalized `AI_OUTPUT_INVALID` failure for that workflow, except for the narrow `interview.evaluate` coaching fallback and persisted-answer report fallback documented below.
  - `interview.report` grounding repair treats submitted answers as the only evidence. Questions, CV/profile/JD context and low-information answers are not positive candidate evidence. After terminal `AI_OUTPUT_INVALID` only, report generation may aggregate READY persisted validated answer evaluations deterministically, then must pass the same report validator. A report may contain zero strengths when no positive claim is grounded; rubric evidence must still be grounded and gaps/action plan remain nonempty. Transport/auth/config/rate-limit failures never select this fallback.
...
- **Asynchronous answer evaluation and question release**:
  - Normal progression uses a prepared server-owned primary-question plan. Answer submission persists the answer, releases the next prepared question and enqueues `interview.evaluate` without calling a provider on the request path.
  - `InterviewAnswerEvaluationRequested` owns durable `queued → processing → ready|failed` state. Idempotent replay cannot duplicate the official answer or effective job. Provider failure preserves the answer and never fabricates an evaluation or question.
  - Prepared questions stay hidden until `ReleasedAt` is set. Adaptive follow-up generation is not part of normal live progression; explicit lineage-based practice modes may still use it.
  - **Report STAR story summary**:
  - Answer evaluation remains independent per answer, but its coaching payload is withheld while the session is active. `report.starSummary` is a deterministic story-level view built from the persisted evaluations; it does not trigger another AI operation.
  ...
  - **Per-answer coaching (`interview.evaluate`)**:
    ...
    - Sample validity is independent of core evaluation validity. Missing, malformed, unsupported-framework, blank or otherwise unusable sample data normalizes to null/absent while an otherwise-valid core evaluation remains usable. Historical evaluation JSON with no sample field remains valid and reads as null/absent.
    - DeepSeek `interview.evaluate` starts with low reasoning and a 6,000-token output budget. A semantic, malformed-JSON, or output-truncation recovery call disables reasoning; only output truncation expands that second call's budget to 8,192. The structured executor still permits at most two provider calls per execution and never accepts a truncated response as valid.
...
content length, structural failure stage, JSON line/byte positions, and correlation ID. The diagnostics
distinguish malformed envelope, missing/invalid choices, message/content shape,
blank content, JSON parse failures, and typed contract deserialization failures; raw model content, prompts,
