...
- **Follow-up Failure Isolation**:
  ...
  - **Per-answer coaching (`interview.evaluate`)**:
    ...
    - If both bounded attempts have valid rubric, evidence, feedback, STAR, strengths and improvements but only `improvedAnswer` remains ungrounded or fabricated, the server replaces that presentation field with a deterministic value containing only the submitted candidate answer (bounded to the existing 4,000-character contract). This adds no provider call and does not recover invalid core evaluation fields. The normalized coaching is persisted with the answer evaluation and remains provider-neutral.
  - **Context Budgeting**:
