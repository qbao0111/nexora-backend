...
namespace Nexora.Api.Observability;
...
public interface IApiSentryReporter
{
    void Capture(Exception exception, string requestId, string method, string path, int statusCode);
}

public sealed class ApiSentryReporter(IHub hub) : IApiSentryReporter
{
    public void Capture(Exception exception, string requestId, string method, string path, int statusCode)
    {
        ...
        hub.CaptureException(exception, scope =>
        {
            ...
            scope.SetTag("http.status_code", statusCode.ToString(System.Globalization.CultureInfo.InvariantCulture));
        });
    }
}
