...
## 2026-09-10 — B8 STAR Story Bank

- Branch: `feat/star-story-bank`; base branch: `feat/scenario-practice-v2` because B7 was not merged into `main` when B8 started. This B8 branch is currently stacked on `feat/scenario-practice-v2`.
- Implementation commit: `ae2d3ef feat(star): implement star story bank`.
- Scope: added the user-owned persistent `StarStory` resource, save/convert from a usable completed STAR attempt, normalized tags, owner-scoped list/detail/search, edit, and explicit re-evaluation using current persisted STAR fields.
- API changes: `POST/GET/PATCH /api/v1/star-stories`, `GET /api/v1/star-stories/{storyId}`, and `POST /api/v1/star-stories/{storyId}/evaluate`; create/evaluate use `Idempotency-Key`, list defaults to `updatedAt DESC`, and ownership returns 404-style semantics. Frontend/API/data-model documentation was updated.
- Migration impact: P1 migration ownership was confirmed for Bảo Nguyên after the P0 CV/Interview migration baseline. Added the narrowly scoped `20260910030000_StarStoryBank` migration and ModelSnapshot changes for `star_stories`, `(UserId, UpdatedAt)` index, and restricted user FK. No unrelated schema changes; no bookmark schema was added.
- STAR evaluation/quota: saved content is grounded in the four existing evaluation evidences; re-evaluation calls the existing `star.evaluate` structured validator against the current persisted story, reserves/consumes `star_builder` quota only on success, voids failed reservations, and preserves the previous valid score on failure. Same-key replay avoids a second AI call or quota consumption.
- Tests: solution build passed with 0 warnings/0 errors and B8 integration tests compile. B8 integration and unit test execution were attempted but the host's Windows Application Control blocked loading unsigned workspace assemblies (`FileLoadException`, 0x800711C7); no assertion failures were observed. `dotnet format --verify-no-changes` also reports pre-existing repository-wide formatting violations; `git diff --check` is clean for tracked changes.
- Dependencies/blockers: B7 remains unmerged; reviewers should review this branch against `feat/scenario-practice-v2`, then retarget/rebase after B7 merges to `main`. EF CLI pending-model verification could not run because the same Application Control policy blocked design-time assembly loading; migration and snapshot were manually verified to contain only B8 schema changes. Interview files/context and `PracticeService.cs` were not modified. Bookmark is deferred.
