...
## 2026-09-11 — A13 Production Config Hardening

- Status: Implementation complete; deterministic validation and three independent audits passed; operator configuration and hosted CI remain pending
- Task/branch: `A13` / `chore/a13-production-config-hardening`
- Base: `483673091c90c47652cb64c061fd75a71790a50a` (`origin/main`, after A11)
- Scope: Centralized Staging/Production startup validation for PostgreSQL, non-default JWT signing keys, private R2 storage, Resend, Sentry DSN/release and strict frontend CORS origins. CORS and auth CSRF checks share the same origin normalization/allowlist helper. Development/Testing retain local/fake adapters where safe; production filesystem storage is rejected even when uploads are disabled.
- Runtime/config: Render staging now selects `Storage__Provider=r2` and supplies R2/Resend/Sentry values through `sync: false` settings. `scripts/render-entrypoint.sh` derives an unset `Sentry__Release` from `RENDER_GIT_COMMIT`, shares the inherited environment with API/Worker, and creates a local storage directory only for the local provider; the image no longer pre-creates a product-storage directory. API and Worker resolve the same release through the shared helper.
- Contracts and docs: `.env.example`, `render.yaml`, `Dockerfile`, `docs/04-production-runbook.md`, `docs/render-staging.md`, `docs/development-setup.md` and the A13 checklist document safe key names, environment differences, R2 privacy, CORS requirements, Render/Railway operational setup and DEC-01 through DEC-04 gates. No frontend origin, DSN, account, bucket or credential is guessed or committed.
- Tests/verification: `dotnet tool restore`; `dotnet restore Nexora.slnx`; Release solution build (0 warnings/errors); Release unit tests (385 passed); Release integration tests (227 passed); focused ProductionSafety/storage tests (64 passed); EF pending-model check reported no changes; NuGet vulnerability audit returned no vulnerable entries; changed-file style/analyzer verification passed; Render Blueprint validation reported `valid: true`; `git diff --check` passed. No live R2, Resend, Sentry, payment or production database calls were made.
- Review hardening: deployed validation rejects documented JWT/Resend/R2 template values and padded R2 endpoints; the Render entrypoint fails closed on mismatched API/Worker environment variables and exports one shared value. Existing no-`Origin` compatibility for non-browser cookie clients and presence-only DSN validation were reviewed and intentionally left unchanged outside this A13 contract.
- Migration/FE impact: None; no EF entity, DbSet, snapshot or migration changed, and `nexora-fe` was untouched.
- Remaining external steps: enter real Postgres, R2, Resend, Sentry DSN, JWT, verified frontend origin and staging AI/payment sandbox values in the deployment dashboard; verify Sentry alert delivery and retention/legal/domain readiness before production. Railway deployment and DEC-01 through DEC-04 remain unapproved/deferred.
- Commit/PR: pending final local review and push; no PR number or commit SHA is asserted in this entry.
