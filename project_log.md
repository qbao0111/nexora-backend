...
## 2026-09-11 — B13 hosted CI corrective validation

- Hosted CI failure inspected: PR #64 Backend CI run `34583818387` at head `00a7f3ac6ea8abfd49830edfe6fc4c6806cab622` built successfully and passed all 378 unit tests, but integration execution reported 244 passed and 6 failed. The primary cause was `ProgressDashboardApiTests.NewFakeFactory` replacing the application-wide `TimeProvider` with fixed `2026-09-16T03:00:00Z`, making hosted-runner JWTs not-yet-valid against the real validation clock and causing 401 responses. The response helper then obscured some failures by calling `GetProperty("data")` before reporting status/body.
- Corrective commit: `d0305b94bc1346713b6c9bf6cb00332bdf0a1066` (`fix(progress): stabilize dashboard integration tests`). The fake factory now retains the normal application `TimeProvider`; the weekly test captures real UTC now, derives the expected Monday boundary through `ProgressDashboardPolicy.GetUtcWeekStart`, and seeds rows relative to that captured instant. The data helper now reports HTTP status and body before accessing the data envelope. The pure fixed-date week-boundary test remains in unit coverage.
- Actual local rerun: focused B13 unit tests passed 10/10. The six named B13 integration tests and the full integration suite could not load the fresh test host because Windows Application Control blocked `Sentry.dll` (`0x800711C7`); the full local integration result was 10 passed and 240 environment-blocked failures, with no B13 assertion result available locally. Full unit execution passed 369 tests; 9 Worker/Sentry tests were blocked by the same host policy.
- Deterministic validation: Release solution build passed with 0 warnings/errors using the temporary untracked removal of the host-blocked Sentry analyzer; EF reported `No changes have been made to the model since the last migration.`; changed-file style and analyzer verification passed; the NuGet vulnerability audit returned no vulnerable entries; `git diff --check` and CRLF-aware diff check passed.
- Scope: No production authentication or JWT validation behavior changed. No migration, ModelSnapshot, `PracticeService.cs`, or B14+ change was made. The corrective branch remains `feat/progress-dashboard-v2`; PR #64 was not merged or auto-merged, and B14 was not started. Hosted CI rerun is required after pushing the corrective commit.

## 2026-09-11 — B13 SQLite weekly-query correction

- Hosted CI failure inspected: PR #64 Backend CI run `34585390741` (run number 68) at head `182167b28e5eb2d998aa04d71f653f15d175030f` built successfully, passed 378/378 unit tests and reported 244/250 integration tests passed. The six B13 dashboard tests returned HTTP 500 `INTERNAL_ERROR` after the JWT clock correction.
- Confirmed root cause: a direct SQLite probe against the real `NexoraDbContext` reproduced `System.InvalidOperationException: The LINQ expression ... could not be translated` for `CompletedAt >= @windowStart && CompletedAt <= @now` on `DateTimeOffset?`. The first failing query in `ProgressDashboardService.GetWeeklyActivitiesAsync` was the owner/status-filtered `ResumeAnalyses` count; the same translation failure was reproduced for `InterviewSessions`, `ScenarioAttempts`, `StarAttempts` and the owner-joined `LearningPathActivities` count. The API middleware correctly kept the response as a safe 500, so no middleware change was made.
- Corrective commit: `ee6ab8128d23f7227184ac3ffc8d90f6c5afe056` (`fix(progress): support sqlite weekly dashboard queries`). PostgreSQL keeps DB-side `CountAsync` range queries. SQLite now applies owner and completed-status plus non-null `CompletedAt` filters at the database boundary, projects only completion timestamps, materializes that owner-scoped projection, and applies the unchanged UTC Monday-start/current-now window comparison in memory. No timestamp type or business semantics changed.
- Regression coverage: `WeeklyCountsUseUtcWeekOwnerBoundaryAndExcludeIncompleteRows` continues through the real `ProgressDashboardService` and now explicitly covers a current-week completed row, prior-week row, another-user row, failed row and completed-status/null-timestamp incomplete row.
